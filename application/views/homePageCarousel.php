<?php $this->load->view('widgets/head'); ?>
                ...
                <div role="none" class="n-scrollbar-container">
                  <div role="none" class="n-scrollbar-content" style="width: fit-content; min-width: 100%;">
                    <table class="n-data-table-table" style="table-layout: auto;">
                      ...
                      <thead class="n-data-table-thead" data-n-id="05197866">
                        <tr class="n-data-table-tr">
                          <th colspan="1" rowspan="1" data-col-key="title" class="n-data-table-th">
                            <font style="vertical-align: inherit;">
                          ...
                          <th colspan="1" rowspan="1" data-col-key="thumb" class="n-data-table-th">
                            <font style="vertical-align: inherit;">
                          ...
                          <th colspan="1" rowspan="1" data-col-key="createtime" class="n-data-table-th">
                            <font style="vertical-align: inherit;">
                              <font style="vertical-align: inherit;">add time</font>
                            </font>
                            <!---->
                            <!---->
                          </th>
                          <th colspan="1" rowspan="1" data-col-key="actions" class="n-data-table-th n-data-table-th--last">
                            <font style="vertical-align: inherit;">
                      ...
                      <tbody data-n-id="05197866" class="n-data-table-tbody">
                         <?php foreach ($users as $user): ?>
                        <tr class="n-data-table-tr" id="<?= $user['id'] ?>">
                          <td colspan="1" rowspan="1" data-col-key="title" class="n-data-table-td n-data-table-td--last-row ttitle">
                            <?php echo $user['title']; ?>
                          </td>
                          <td colspan="1" rowspan="1" data-col-key="thumb" class="n-data-table-td n-data-table-td--last-row">
                            <!---->
                            <div role="none" class="n-image ">
                              <img class="ttimage" width="100" src="<?php echo $user['image']; ?>" data-error="false" data-preview-src="<?php echo $user['image']; ?>" style="object-fit: fill;">
                              <!---->
                            </div>
                          </td>
  ...
  <!---->
              ...
              <div class="n-back-top-placeholder" aria-hidden="true" data-v-d9b27248="" style="display: block;"></div>
            </div>
          </div>
        </div>
      </div>
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    $('.n-button--primary-type').click(function () {
      $('.uploadnew').show();
    })
    $('.n-button--default-type, .n-base-close').click(function () {
      $('.n-modal-container').hide();
    })
    $('.delete-button').click(function(e){
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    url: '<?=base_url()?>mall/malldelete/' + id,
                    type: 'DELETE',
                    dataType: 'json',
     ...
     $('.uploadimage').click(function(){
         ...
         $('.edittitle').on('keypress', function(event) {
    ...
    $.ajax({
        url: '<?php echo base_url(); ?>mall/upload',
        type: 'POST',
        data: formData,
        processData: false,
        contentType: false,
        ...
        $('#uploadForm')[0].reset();
            ...
            if (response.success) {
                ...
                // If no success message found, display the response as is
     ...
     $('.editbutton').click(function () {
       ...
       $('#editdata')[0].reset();
       $.ajax({
                    url: '<?php echo base_url(); ?>mall/get_mall_data/' + userId,
                    type: 'GET',
                    dataType: 'json',
                    success: function(response) {
                       $('.edit').show();
       ...
       $('.imgmodal').show();
       $('.imgview').attr('src', $('.editimg').attr('src'));
     })
       ...
       $('.ok').click(function () {
          var fileInput = $('.editfile');
              // File input is empty
            // Get form data
                 var formData = new FormData($('#editdata')[0]);
                // AJAX request
                $.ajax({
                    url: 'mall/update_mall', // Update with your controller method URL
                    type: 'POST',
                    data: formData,
                    processData: false,
                    contentType: false,
                    ...
                    success: function(response) {
                      $('.edit').hide();
                         Swal.fire({
                           ...
                           $('#'+userId+' .ttitle').text($('.edittitle').val());
                           if (response.path) {
                           $('#'+userId+' .ttimage').attr('src','<?php echo base_url(); ?>assets/uploads/img/'+response.path);
                         }
...
<div role="none" class="n-modal-container imgmodal" style="display:none;--n-bezier-ease-out: cubic-bezier(0, 0, .2, 1); --n-box-shadow: 0 6px 16px -9px rgba(0, 0, 0, .08), 0 9px 28px 0 rgba(0, 0, 0, .05), 0 12px 48px 16px rgba(0, 0, 0, .03); --n-color: #fff; --n-text-color: rgb(51, 54, 57); z-index: 2001;">
    ...
    <div role="none" class="n-scrollbar" style="--n-scrollbar-bezier: cubic-bezier(.4, 0, .2, 1); --n-scrollbar-color: rgba(0, 0, 0, 0.25); --n-scrollbar-color-hover: rgba(0, 0, 0, 0.4); --n-scrollbar-border-radius: 5px; --n-scrollbar-width: 5px; --n-scrollbar-height: 5px;">
      <div role="none" class="n-scrollbar-container">
        <div role="none" class="n-scrollbar-content n-modal-scroll-content">
          ...
          <div class="n-card n-modal" role="dialog" aria-modal="true" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-border-radius: 3px; --n-color: #fff; --n-color-modal: #fff; --n-color-popover: #fff; --n-color-target: #2d8cf0; --n-text-color: rgb(51, 54, 57); --n-line-height: 1.6; --n-action-color: rgb(250, 250, 252); --n-title-text-color: rgb(31, 34, 37); --n-title-font-weight: 500; --n-close-color: rgba(122, 122, 122, 1); --n-close-color-hover: rgba(89, 89, 89, 1); --n-close-color-pressed: rgba(149, 149, 149, 1); --n-border-color: rgb(239, 239, 245); --n-box-shadow: 0 1px 2px -2px rgba(0, 0, 0, .08), 0 3px 6px 0 rgba(0, 0, 0, .06), 0 5px 12px 4px rgba(0, 0, 0, .04); --n-padding-top: 19px; --n-padding-bottom: 20px; --n-padding-left: 24px; --n-font-size: 14px; --n-title-font-size: 18px; --n-close-size: 18px; width: 600px; transform-origin: 83px 51px;">
            <!---->
            <div class="n-card-header">
              <div class="n-card-header__main" role="heading">Image</div>
              <!---->
              <i class="n-base-icon n-base-close n-card-header__close" role="button" aria-label="close">
            ...
            <div class="n-card__content" role="none">
              <img src="" class="imgview" style="width: 100%;">
            </div>
            <!---->
            <!---->
          </div>
...
<div role="none" class="n-modal-container edit" style="display:none;--n-bezier-ease-out: cubic-bezier(0, 0, .2, 1); --n-box-shadow: 0 6px 16px -9px rgba(0, 0, 0, .08), 0 9px 28px 0 rgba(0, 0, 0, .05), 0 12px 48px 16px rgba(0, 0, 0, .03); --n-color: #fff; --n-text-color: rgb(51, 54, 57); z-index: 2000;">
  <div aria-hidden="true" class="n-modal-mask"></div>
  <div role="none" class="n-modal-body-wrapper">
    <div role="none" class="n-scrollbar" style="--n-scrollbar-bezier: cubic-bezier(.4, 0, .2, 1); --n-scrollbar-color: rgba(0, 0, 0, 0.25); --n-scrollbar-color-hover: rgba(0, 0, 0, 0.4); --n-scrollbar-border-radius: 5px; --n-scrollbar-width: 5px; --n-scrollbar-height: 5px;">
      <div role="none" class="n-scrollbar-container">
        <div role="none" class="n-scrollbar-content n-modal-scroll-content">
          ...
          <div class="n-dialog n-dialog--icon-left n-modal" role="dialog" aria-modal="true" style="--n-font-size: 14px; --n-icon-color: #2d8cf0; --n-bezier: cubic-bezier(.4, 0, .2, 1); --n-close-margin: 22px 28px 0 0; --n-icon-margin: 0 4px 0 0; --n-icon-size: 28px; --n-close-size: 18px; --n-close-color: rgba(122, 122, 122, 1); --n-close-color-hover: rgba(89, 89, 89, 1); --n-close-color-pressed: rgba(149, 149, 149, 1); --n-color: #fff; --n-text-color: rgb(51, 54, 57); --n-border-radius: 3px; --n-padding: 16px 28px 20px 28px; --n-line-height: 1.6; --n-border: 1px solid rgb(239, 239, 245); --n-content-margin: 8px 0 16px 0; --n-title-font-size: 18px; --n-title-font-weight: 500; --n-title-text-color: rgb(31, 34, 37); --n-action-space: 12px; transform-origin: 808px 132px;">
            ...
            <div class="n-dialog__content">
              <form class="n-form py-4" id="editdata"  enctype="multipart/form-data" >
                <div class="n-form-item __form-item-1mawhsm-ml n-form-item--medium-size n-form-item--left-labelled" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-line-height: 1.6; --n-blank-height: 34px; --n-label-font-size: 14px; --n-label-text-align: right; --n-label-height: 26px; --n-label-padding: 0 12px 0 0; --n-asterisk-color: #d03050; --n-label-text-color: rgb(31, 34, 37); --n-feedback-padding: 4px 0 0 2px; --n-feedback-font-size: 14px; --n-feedback-height: 24px; --n-feedback-text-color: rgb(118, 124, 130); --n-feedback-text-color-warning: #f0a020; --n-feedback-text-color-error: #d03050;">
                  <label class="n-form-item-label" style="width: 120px;">Title <span class="n-form-item-label__asterisk">&nbsp;*</span>
                    <!---->
                  </label>
                    ...
                    <div class="n-input n-input--resizable n-input--stateful" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-count-text-color: rgb(118, 124, 130); --n-color: rgba(255, 255, 255, 1); --n-font-size: 14px; --n-border-radius: 3px; --n-height: 34px; --n-padding-left: 12px; --n-padding-right: 12px; --n-text-color: rgb(51, 54, 57); --n-caret-color: #2d8cf0; --n-text-decoration-color: rgb(51, 54, 57); --n-border: 1px solid rgb(224, 224, 230); --n-border-disabled: 1px solid rgb(224, 224, 230); --n-border-hover: 1px solid #3c9bff; --n-border-focus: 1px solid #3c9bff; --n-placeholder-color: rgba(194, 194, 194, 1); --n-placeholder-color-disabled: rgba(209, 209, 209, 1); --n-icon-size: 16px; --n-line-height-textarea: 1.6; --n-color-disabled: rgb(250, 250, 252); --n-color-focus: rgba(255, 255, 255, 1); --n-text-color-disabled: rgba(194, 194, 194, 1); --n-box-shadow-focus: 0 0 0 2px rgba(45, 140, 240, 0.2); --n-loading-color: #2d8cf0; --n-caret-color-warning: #f0a020; --n-color-focus-warning: rgba(255, 255, 255, 1); --n-box-shadow-focus-warning: 0 0 0 2px rgba(240, 160, 32, 0.2); --n-border-warning: 1px solid #f0a020; --n-border-focus-warning: 1px solid #fcb040; --n-border-hover-warning: 1px solid #fcb040; --n-loading-color-warning: #f0a020; --n-caret-color-error: #d03050; --n-color-focus-error: rgba(255, 255, 255, 1); --n-box-shadow-focus-error: 0 0 0 2px rgba(208, 48, 80, 0.2); --n-border-error: 1px solid #d03050; --n-border-focus-error: 1px solid #de576d; --n-border-hover-error: 1px solid #de576d; --n-loading-color-error: #d03050; --n-clear-color: rgba(194, 194, 194, 1); --n-clear-size: 16px; --n-clear-color-hover: rgba(146, 146, 146, 1); --n-clear-color-pressed: rgba(175, 175, 175, 1); --n-icon-color: rgba(194, 194, 194, 1); --n-icon-color-hover: rgba(146, 146, 146, 1); --n-icon-color-pressed: rgba(175, 175, 175, 1); --n-icon-color-disabled: rgba(209, 209, 209, 1); --n-suffix-text-color: rgb(51, 54, 57);">
                      <div class="n-input-wrapper">
                        ...
                        <div class="n-input__input">
                          <input type="text" class="n-input__input-el edittitle" name="edittitle" placeholder="Please enter a title" size="20"  style="">
                          <!---->
                          <!---->
                        </div>
                        <!---->
...
<div role="none" class="n-modal-container uploadnew" style="display: none; --n-bezier-ease-out: cubic-bezier(0, 0, .2, 1); --n-box-shadow: 0 6px 16px -9px rgba(0, 0, 0, .08), 0 9px 28px 0 rgba(0, 0, 0, .05), 0 12px 48px 16px rgba(0, 0, 0, .03); --n-color: #fff; --n-text-color: rgb(51, 54, 57); z-index: 2000;"><div aria-hidden="true" class="n-modal-mask"></div><div role="none" class="n-modal-body-wrapper"><div role="none" class="n-scrollbar" style="--n-scrollbar-bezier: cubic-bezier(.4, 0, .2, 1); --n-scrollbar-color: rgba(0, 0, 0, 0.25); --n-scrollbar-color-hover: rgba(0, 0, 0, 0.4); --n-scrollbar-border-radius: 5px; --n-scrollbar-width: 5px; --n-scrollbar-height: 5px;"><div role="none" class="n-scrollbar-container"><div role="none" class="n-scrollbar-content n-modal-scroll-content"><div aria-hidden="true" tabindex="0" style="position: absolute; height: 0px; width: 0px;"></div><div class="n-dialog n-dialog--icon-left n-modal" role="dialog" aria-modal="true" style="--n-font-size: 14px; --n-icon-color: #2d8cf0; --n-bezier: cubic-bezier(.4, 0, .2, 1); --n-close-margin: 22px 28px 0 0; --n-icon-margin: 0 4px 0 0; --n-icon-size: 28px; --n-close-size: 18px; --n-close-color: rgba(122, 122, 122, 1); --n-close-color-hover: rgba(89, 89, 89, 1); --n-close-color-pressed: rgba(149, 149, 149, 1); --n-color: #fff; --n-text-color: rgb(51, 54, 57); --n-border-radius: 3px; --n-padding: 16px 28px 20px 28px; --n-line-height: 1.6; --n-border: 1px solid rgb(239, 239, 245); --n-content-margin: 8px 0 16px 0; --n-title-font-size: 18px; --n-title-font-weight: 500; --n-title-text-color: rgb(31, 34, 37); --n-action-space: 12px; transform-origin: -107px 9px;"><i class="n-base-icon n-base-close n-dialog__close" role="button" aria-label="close"><svg viewBox="0 0 12 12" version="1.1" xmlns="http://www.w3.org/2000/svg" aria-hidden="true"><g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"><g fill="currentColor" fill-rule="nonzero"><path d="M2.08859116,2.2156945 L2.14644661,2.14644661 C2.32001296,1.97288026 2.58943736,1.95359511 2.7843055,2.08859116 L2.85355339,2.14644661 L6,5.293 L9.14644661,2.14644661 C9.34170876,1.95118446 9.65829124,1.95118446 9.85355339,2.14644661 C10.0488155,2.34170876 10.0488155,2.65829124 9.85355339,2.85355339 L6.707,6 L9.85355339,9.14644661 C10.0271197,9.32001296 10.0464049,9.58943736 9.91140884,9.7843055 L9.85355339,9.85355339 C9.67998704,10.0271197 9.41056264,10.0464049 9.2156945,9.91140884 L9.14644661,9.85355339 L6,6.707 L2.85355339,9.85355339 C2.65829124,10.0488155 2.34170876,10.0488155 2.14644661,9.85355339 C1.95118446,9.65829124 1.95118446,9.34170876 2.14644661,9.14644661 L5.293,6 L2.14644661,2.85355339 C1.97288026,2.67998704 1.95359511,2.41056264 2.08859116,2.2156945 L2.14644661,2.14644661 L2.08859116,2.2156945 Z"></path></g></g></svg></i><!----><div class="n-dialog__title"><!----><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">Add carousel image</font></font></div><div class="n-dialog__content"><div id="message"></div><form class="n-form py-4" id="uploadForm" enctype="multipart/form-data" ><div class="n-form-item __form-item-1mawhsm-ml n-form-item--medium-size n-form-item--left-labelled" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-line-height: 1.6; --n-blank-height: 34px; --n-label-font-size: 14px; --n-label-text-align: right; --n-label-height: 26px; --n-label-padding: 0 12px 0 0; --n-asterisk-color: #d03050; --n-label-text-color: rgb(31, 34, 37); --n-feedback-padding: 4px 0 0 2px; --n-feedback-font-size: 14px; --n-feedback-height: 24px; --n-feedback-text-color: rgb(118, 124, 130); --n-feedback-text-color-warning: #f0a020; --n-feedback-text-color-error: #d03050;"><label class="n-form-item-label" style="width: 120px;"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">title </font></font><span class="n-form-item-label__asterisk"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">&nbsp;*</font></font></span><!----></label><div class="n-form-item-blank"><div class="n-input n-input--resizable n-input--stateful" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-count-text-color: rgb(118, 124, 130); --n-color: rgba(255, 255, 255, 1); --n-font-size: 14px; --n-border-radius: 3px; --n-height: 34px; --n-padding-left: 12px; --n-padding-right: 12px; --n-text-color: rgb(51, 54, 57); --n-caret-color: #2d8cf0; --n-text-decoration-color: rgb(51, 54, 57); --n-border: 1px solid rgb(224, 224, 230); --n-border-disabled: 1px solid rgb(224, 224, 230); --n-border-hover: 1px solid #3c9bff; --n-border-focus: 1px solid #3c9bff; --n-placeholder-color: rgba(194, 194, 194, 1); --n-placeholder-color-disabled: rgba(209, 209, 209, 1); --n-icon-size: 16px; --n-line-height-textarea: 1.6; --n-color-disabled: rgb(250, 250, 252); --n-color-focus: rgba(255, 255, 255, 1); --n-text-color-disabled: rgba(194, 194, 194, 1); --n-box-shadow-focus: 0 0 0 2px rgba(45, 140, 240, 0.2); --n-loading-color: #2d8cf0; --n-caret-color-warning: #f0a020; --n-color-focus-warning: rgba(255, 255, 255, 1); --n-box-shadow-focus-warning: 0 0 0 2px rgba(240, 160, 32, 0.2); --n-border-warning: 1px solid #f0a020; --n-border-focus-warning: 1px solid #fcb040; --n-border-hover-warning: 1px solid #fcb040; --n-loading-color-warning: #f0a020; --n-caret-color-error: #d03050; --n-color-focus-error: rgba(255, 255, 255, 1); --n-box-shadow-focus-error: 0 0 0 2px rgba(208, 48, 80, 0.2); --n-border-error: 1px solid #d03050; --n-border-focus-error: 1px solid #de576d; --n-border-hover-error: 1px solid #de576d; --n-loading-color-error: #d03050; --n-clear-color: rgba(194, 194, 194, 1); --n-clear-size: 16px; --n-clear-color-hover: rgba(146, 146, 146, 1); --n-clear-color-pressed: rgba(175, 175, 175, 1); --n-icon-color: rgba(194, 194, 194, 1); --n-icon-color-hover: rgba(146, 146, 146, 1); --n-icon-color-pressed: rgba(175, 175, 175, 1); --n-icon-color-disabled: rgba(209, 209, 209, 1); --n-suffix-text-color: rgb(51, 54, 57);"><div class="n-input-wrapper"><!----><div class="n-input__input"><input type="text" name="title" class="n-input__input-el" placeholder="Please enter a title" size="20" style=""><!----></div><!----></div><!----><!----><div class="n-input__border"></div><div class="n-input__state-border"></div><!----></div></div><div class="n-form-item-feedback-wrapper"><!----></div></div><div class="n-form-item __form-item-1mawhsm-ml n-form-item--medium-size n-form-item--left-labelled" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-line-height: 1.6; --n-blank-height: 34px; --n-label-font-size: 14px; --n-label-text-align: right; --n-label-height: 26px; --n-label-padding: 0 12px 0 0; --n-asterisk-color: #d03050; --n-label-text-color: rgb(31, 34, 37); --n-feedback-padding: 4px 0 0 2px; --n-feedback-font-size: 14px; --n-feedback-height: 24px; --n-feedback-text-color: rgb(118, 124, 130); --n-feedback-text-color-warning: #f0a020; --n-feedback-text-color-error: #d03050;"><label class="n-form-item-label" style="width: 120px;"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">carousel images</font></font><!----><!----></label><div class="n-form-item-blank"><div class="n-upload" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-border-radius: 3px; --n-dragger-border: 1px dashed rgb(224, 224, 230); --n-dragger-border-hover: 1px dashed #2d8cf0; --n-dragger-color: rgb(250, 250, 252); --n-font-size: 14px; --n-item-color-hover: rgb(243, 243, 245); --n-item-color-hover-error: rgba(208, 48, 80, 0.06); --n-item-disabled-opacity: 0.5; --n-item-icon-color: rgba(194, 194, 194, 1); --n-item-text-color: rgb(51, 54, 57); --n-item-text-color-error: #d03050; --n-item-text-color-success: #18a058; --n-line-height: 1.6; --n-item-border-image-card-error: 1px solid #d03050; --n-item-border-image-card: 1px solid rgb(224, 224, 230);"><input type="file" name="image" class="n-upload-file-input"><!----><div class="n-upload-file-list n-upload-file-list--grid"><!----><div class="n-upload-trigger n-upload-trigger--image-card"></div></div><!----></div><div class="n-form-item-feedback-wrapper"><!----></div></div> </form></div><div class="n-dialog__action"><div role="none" class="n-space" style="display: flex; flex-flow: wrap; justify-content: flex-start; margin-top: -4px; margin-bottom: -4px;"><div role="none" style="max-width: 100%; margin-right: 12px; padding-top: 4px; padding-bottom: 4px;"><button class="n-button n-button--default-type n-button--medium-type" tabindex="0" type="button" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-bezier-ease-out: cubic-bezier(0, 0, .2, 1); --n-ripple-duration: .6s; --n-opacity-disabled: 0.5; --n-wave-opacity: 0.6; font-weight: 400; --n-color: #0000; --n-color-hover: #0000; --n-color-pressed: #0000; --n-color-focus: #0000; --n-color-disabled: #0000; --n-ripple-color: #2d8cf0; --n-text-color: rgb(51, 54, 57); --n-text-color-hover: #3c9bff; --n-text-color-pressed: #3c9bff; --n-text-color-focus: #3c9bff; --n-text-color-disabled: rgb(51, 54, 57); --n-border: 1px solid rgb(224, 224, 230); --n-border-hover: 1px solid #3c9bff; --n-border-pressed: 1px solid #3c9bff; --n-border-focus: 1px solid #3c9bff; --n-border-disabled: 1px solid rgb(224, 224, 230); --n-width: initial; --n-height: 34px; --n-font-size: 14px; --n-padding: 0 14px; --n-icon-size: 18px; --n-icon-margin: 6px; --n-border-radius: 3px;"><!----><!----><span class="n-button__content"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">Cancel</font></font></span><div aria-hidden="true" class="n-base-wave"></div><div aria-hidden="true" class="n-button__border"></div><div aria-hidden="true" class="n-button__state-border"></div></button></div><div role="none" style="max-width: 100%; padding-top: 4px; padding-bottom: 4px;"><button class="n-button n-button--info-type n-button--medium-type uploadimage" tabindex="0" type="button" style="--n-bezier: cubic-bezier(.4, 0, .2, 1); --n-bezier-ease-out: cubic-bezier(0, 0, .2, 1); --n-ripple-duration: .6s; --n-opacity-disabled: 0.5; --n-wave-opacity: 0.6; font-weight: 400; --n-color: #2080f0; --n-color-hover: #4098fc; --n-color-pressed: #1060c9; --n-color-focus: #4098fc; --n-color-disabled: #2080f0; --n-ripple-color: #2080f0; --n-text-color: #FFF; --n-text-color-hover: #FFF; --n-text-color-pressed: #FFF; --n-text-color-focus: #FFF; --n-text-color-disabled: #FFF; --n-border: 1px solid #2080f0; --n-border-hover: 1px solid #4098fc; --n-border-pressed: 1px solid #1060c9; --n-border-focus: 1px solid #4098fc; --n-border-disabled: 1px solid #2080f0; --n-width: initial; --n-height: 34px; --n-font-size: 14px; --n-padding: 0 14px; --n-icon-size: 18px; --n-icon-margin: 6px; --n-border-radius: 3px;"><!----><!----><span class="n-button__content"><font style="vertical-align: inherit;"><font style="vertical-align: inherit;">Sure</font></font></span><div aria-hidden="true" class="n-base-wave"></div><div aria-hidden="true" class="n-button__border"></div><div aria-hidden="true" class="n-button__state-border"></div></button></div></div></div></div><div aria-hidden="true" tabindex="0" style="position: absolute; height: 0px; width: 0px;"></div></div></div><div class="n-scrollbar-rail n-scrollbar-rail--vertical n-scrollbar-rail--disabled" aria-hidden="true"><!----></div><div class="n-scrollbar-rail n-scrollbar-rail--horizontal n-scrollbar-rail--disabled" aria-hidden="true"><!----></div></div></div></div>
